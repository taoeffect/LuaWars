...
public final class GL15 {
	...
	public static ByteBuffer glMapBuffer(int target, int access, ByteBuffer old_buffer) {
		...
		ByteBuffer __result = nglMapBuffer(target, access, GLChecks.getBufferObjectSize(caps, target), old_buffer, function_pointer);
	...
	public static ByteBuffer glGetBufferPointer(int target, int pname) {
		...
		BufferChecks.checkFunctionAddress(function_pointer);
		ByteBuffer __result = nglGetBufferPointerv(target, pname, GLChecks.getBufferObjectSize(caps, target), function_pointer);
		return LWJGLUtil.CHECKS && __result == null ? null : __result.order(ByteOrder.nativeOrder());
	}
